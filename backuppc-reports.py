...
#
# backuppc-reports -- A simple script to interact with the BackupPc summary interface from command line.
#
...
def get_backup_data():
    ...
    for row in table.find_all('tr')[1:]:
        ...
        # ['Host', 'User', '#Full', 'Full Age (days)', 'Full Size (GB)', 'Speed (MB/s)', '#Incr', 'Incr Age (days)', 
        # 'Last Backup (days)', 'State', '#Xfer errs', 'Last attempt']
        # print(f'Host: { row_data[0] } - Last Backup: { row_data[8] } days ago.')
        backuppc_host = {                                                                                                                                                                                
            'hostname': row_data[0],
            'user': row_data[1],
...
def sendEmail(subject, body):
    ...
    try:
        ...
        print(f'Error trying to send mail. Please check your SMTP settings.')
        print(f'{ e }')
    s.quit()    
    return
...
if (__name__ == "__main__"):
    ...
    parser.add_argument("-c", "--config", default='/opt/backuppc-reports/backuppc-reports.cfg', help="Config file full path.")
    subparser = parser.add_subparsers(dest='command')
    summary = subparser.add_parser('summary', help='Show a summary of all hosts in BackupPc.')
    monitor = subparser.add_parser('monitor', help='Check that there are no failed backups and if there are, send an alert by email.')
    version = subparser.add_parser('version', help='Display the current version and exit.')
    checkhost = subparser.add_parser('hostinfo', help='Show a report about backups for a particular host.')
    ...
    try:
        ...
        max_full_backup_age = int(config_parser.get('general', 'max_full_backup_age'))
        max_incr_backup_age = int(config_parser.get('general', 'max_incr_backup_age'))
        last_backup_age = int(config_parser.get('general', 'last_backup_age'))
        ...
        print("FATAL ERROR: There are errors in the configuration file. Please, verify that everything is ok there.")
    ...
    if args.command == 'summary':
        ...
        if host_data == None:
            ...
            if float(host_data["full_bkp_age"]) > float(max_full_backup_age):
                table.rows.append(["Last Full Backups Age",colored(host_data["full_bkp_age"], 'red')])            
            else:
                table.rows.append(["Last Full Backups Age",host_data["full_bkp_age"]])
            table.rows.append(["Full Backup Size (Gb)",host_data["full_size_gb"]])        
            table.rows.append(["# Incremental Backups",host_data["num_incr_bkps"]])        
            if float(host_data["incr_bkp_age"]) > float(max_incr_backup_age):
                table.rows.append(["Last Incremental Backup Age",colored(host_data["incr_bkp_age"], 'red')])        
            else:            
                table.rows.append(["Last Incremental Backup Age",host_data["incr_bkp_age"]])        
            table.rows.append(["Last Backup (Days Ago)",host_data["last_bkp_days"]])
            table.rows.append(["Host State",host_data["state"]])                
            table.rows.append(["XFER Errors in Last Backup",host_data["xfer_errors"]])                
        ...
        for host_data in backuppc_data:
            ...
            if host_data["state"] == 'auto disabled':
                ignored_host += 1
                print('Ignoring host %s - disabled by user.' % (host_data["hostname"]))
            else:
                if float(host_data["full_bkp_age"]) > float(max_full_backup_age):
                    errors += 1
                    error_msg = error_msg + "\nERROR: The last full backup of server '%s' is older (%s days) than expected (%s days)." % (host_data["hostname"], host_data["full_bkp_age"], max_full_backup_age)
                if float(host_data["incr_bkp_age"]) > float(max_incr_backup_age):
                    errors += 1
                    error_msg = error_msg + "\nERROR: The last incremental backup of server '%s' is older (%s days) than expected (%s days)." % (host_data["hostname"], host_data["incr_bkp_age"], max_incr_backup_age)
                if float(host_data["last_bkp_days"]) > float(last_backup_age):
                    errors += 1
                    error_msg = error_msg + "\nERROR: The last backup of server '%s' is older (%s days) than expected (%s days)." % (host_data["hostname"], host_data["last_bkp_days"], last_backup_age)
                if errors > 0:
                    sendEmail('BackupPc Monitor - Problems found with server %s' %(host_data["hostname"]), error_msg)
                    print(error_msg)
                    host_with_errors += 1
        ...
        print('Error: Invalid command. Please use --help to see the available commands.')
